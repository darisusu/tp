...
- **Format:** `add n/NAME p/PHONE e/EMAIL a/ADDRESS dl/DEADLINE paid/PAID s/SCHEDULE [goal/GOAL] [h/HEIGHT] [w/WEIGHT] [age/AGE] [g/GENDER] [bf/BODYFAT] [t/TAG]…`
...
- **Guidance:**
    - Provide all compulsory prefixes: `n/`, `p/`, `e/`, `a/`, `dl/`, `paid/`, and `s/`. Missing any of these will cause the command to fail.
...
### `session` — update the scheduled training session
- **Format:** `session INDEX s/SESSION`
- **Example:** `session 1 s/WEEKLY:MON-1800-1930`
- **Guidance:**
    - Accepts the same session formats described below (one-off, weekly, biweekly, monthly).
    - FitBook prevents overlapping sessions across clients and will reject conflicting schedules.
